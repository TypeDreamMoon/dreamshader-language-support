...
DreamShaderLang `.dsm` 材质文件和 `.dsh` 共享头文件的 VS Code 语言支持扩展。
...
`1.4.0` 版本重写了语言服务核心。补全、诊断、文档符号、折叠和语义高亮现在走 scanner / parser / context 管线，不再依赖旧的正则堆叠逻辑，区块和作用域判断会更稳定。
